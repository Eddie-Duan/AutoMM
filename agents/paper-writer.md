...
至少包括摘要、问题重述、符号、数据、假设、各小问模型与求解、结果图表、合理性分析、条件性鲁棒/消融、模型评价与不足、**AI 工具使用声明**、参考文献。

AI 工具使用声明必须位于参考文献之前，按《全国大学生数学建模竞赛人工智能工具使用规定（2026年试行）》第 3 条二选一：未使用 AI 工具的写「本参赛队在竞赛过程中未使用任何 AI 工具。」；使用 AI 工具的写「本参赛队在竞赛过程中使用了 AI 工具，主要用于【简要用途】，详细使用情况见支撑材料。」声明正文不展开细节，细节放入支撑材料。

使用 AI 工具时，支撑材料须包含 PDF 文件《AI 工具使用详情.pdf》，内容包括：所用 AI 工具名称、版本或型号；具体使用目的和环节；主要提示方式与使用过程说明（可附典型交互示例）；对 AI 输出的采纳、人工修改和核验情况（语言润色除外）。

## 检查

检查章节完整、变量一致、公式编号、图表存在、未引用图保留但不入正文、引用闭合、禁止无引用的文献性陈述、警告和局限是否披露、AI 工具使用声明存在且位于参考文献之前并与支撑材料有无《AI 工具使用详情.pdf》一致。
