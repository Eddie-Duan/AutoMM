...
引理 1 说明“不需要二元变量”是精确性质而非近似；其副作用是最优解可能不唯一，本文按“模型假设”一节约定的规则提交解，以保证结果可复现。同一现象在电池调度文献中亦被讨论：当往返效率小于 1 时线性松弛的互补性可以自发满足，而强制互补会带来数值与规模上的额外代价\cite{ref-wang-2024-complementarity,ref-nazir-2023-realizable-dispatch}。
...
全部模型均为线性规划，用通用求解器在普通 CPU 上即可直接求解，不需要特殊硬件：问题二的全年单次线性规划约需 5.8 s，问题三与链 \emph{4-3} 因分解为 1460 个单日问题，分别约需 42 s 与 39 s，链 \emph{4-2} 的逐日递推约需 12 s。

为确认结果可靠，本文对每个问题都做了相互独立的复核。一是\textbf{独立回代}：用原始附件与解序列重新计算平衡方程、状态转移与各项约束，不复用求解器自带的检验结果，也不重新求解；二是\textbf{量纲与逐层目标复算}，并把交付表格逐格与解序列比对；三是\textbf{解析界与常识检验}，例如用“完全不使用储能”的策略构造费用上界，并说明因附件 4 存在极端低价、由最小电价构造的下界不具判别力；四是\textbf{跨问题一致性}，净负荷、弃光量这类与价格无关的量在四问之间逐位相同，而费用必然不同。四问的等式残差均不超过 $4.6\times10^{-12}$，容量与功率约束零越界，交付工作簿逐格复算一致。
